    ...
    public class StatisticController : Controller
    {
        ...
        public ActionResult GoodSubjectStatisticsAnalysis(FormCollection collection)
        {
            if (collection["OutGoodSubjectStatistics"] != null)
            {
                ...
                DataTable dt = new DataTable();
                string excelName = "政府采购统计分析列表";
                string fileName = DateTime.Now.ToString("yyyyMMddhhmmss");
                string path = System.Web.HttpContext.Current.Server.MapPath("~/UploadFiles/政府采购-统计分析-" + fileName + ".xls");
                //Excel表头
                dt.Columns.Add("序号");
                dt.Columns.Add("项目年份");
                dt.Columns.Add("项目类型");
                ...
                return this.File(path, "application/octet-stream", "政府采购-统计分析-" + fileName + ".xls");
