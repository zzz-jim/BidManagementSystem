...
namespace UI.ScientificResearch.Controllers
{
    ...
    public class ResearchAwardController : Controller
    {
        ...
        public ResearchAwardController()
            : this(
        ...
        {
            int formId = (int)ResearchAwardTypeOfFormId.Application;
            int workflowId = (int)ResearchAwardTypeOfWorkflowId.Application;
            ...
            else//新建申请书的view
            {
                ...
                // 自动生成 编号
                model.BeiYong1 = CommonHelper.GenerateProjectNumber(ApplicationType.ResearchAward);
        ...
        public ActionResult SignContract(string id, string nextaction)
        {
            ...
            {
                int formId = (int)ResearchAwardTypeOfFormId.Contract;
                int workflowId = (int)ResearchAwardTypeOfWorkflowId.Contract;
                //加载表单内容
                ...
                var result = ApplicationService.GetEntities(p => p.ApplicationId == applicaionId && p.FormID == (int)ResearchAwardTypeOfFormId.Contract).ToList();
        ...
        public ActionResult ProcessRecords(string id, string nextaction)
        {
            ...
            if (workToDoModel.ApplicationId == 0)
            {
                ...
                int formId = (int)ResearchAwardTypeOfFormId.ProcessRecord;
                int workflowId = (int)ResearchAwardTypeOfWorkflowId.ProcessRecord;
        ...
        public ActionResult ReimburseProcess(string id, string nextaction)
        {
            ...
            if (nextaction == "save")
            {
                ...
                using (var context = new CSPostOAEntities())
                {
                    ViewBag.FundsType = (from h in context.FundsThreshold
                                         where (h.IsDeleted == false && h.ModuleName == ModuleNameOfScienceResearch.ResearchAward.ToString() && h.ProjectType == projectType)
            ...
            else
            {
                ...
                int workflowId = (int)ResearchAwardTypeOfWorkflowId.FeeReimbursement;
                model.CreatedTime = DateTime.Now;
        ...
        public ActionResult TravelExpenses(string id, string nextaction)
        {
            ...
            {
                TravelFundsRecordViewModel model = new TravelFundsRecordViewModel();
                int workflowId = (int)ResearchAwardTypeOfWorkflowId.TravelReimbursement;
        ...
        public ActionResult ConclusionsAgree(string id, string nextaction)
        {
            ...
            var model = ApplicationService.GetEntities(p => p.ApplicationId == id1 && p.FormID == Convert.ToInt32(ResearchAwardTypeOfFormId.Conclusion)).FirstOrDefault().ToViewModel();
            //继续审批
        ...
        public ActionResult Conclusions(string id, string nextaction)
        {
            ...
            //判断是否已经进入到项目结题审批中
            var applicaitonodelcount = ApplicationService.GetEntities(p => p.ApplicationId == applicationid && p.FormID == (int)ResearchAwardTypeOfFormId.Conclusion).Count();

            if (applicaitonodelcount > 0)
            {
                ERPNWorkToDoViewModel model = new ERPNWorkToDoViewModel();
                model = ApplicationService.GetEntities(p => p.ApplicationId == applicationid && p.FormID == Convert.ToInt32(ResearchAwardTypeOfFormId.Conclusion)).FirstOrDefault().ToViewModel();
            ...
            if (nextaction == "save")
            {
                ...
                ViewBag.act = "save";
                ERPNWorkToDoViewModel model = new ERPNWorkToDoViewModel();
                model = ApplicationService.GetEntities(p => p.FormID == (int)ResearchAwardTypeOfFormId.Conclusion && p.ApplicationId == applicationid).FirstOrDefault().ToViewModel();
                return View(model);
            ...
            {
                int formId = (int)ResearchAwardTypeOfFormId.Conclusion;
                int workflowId = (int)ResearchAwardTypeOfWorkflowId.Conclusion;
        ...
        public ActionResult ExtensionRequest(string id, string nextaction)
        {
            ...
            int countOfConclusion = ApplicationService.GetEntities(p => p.ApplicationId == applicationid && p.FormID == (int)ResearchAwardTypeOfFormId.Conclusion&&p.IsTemporary!=true).Count();
            ...
            int countOfExtensionRequest = ApplicationService.GetEntities(p => p.ApplicationId == applicationid && p.FormID == (int)ResearchAwardTypeOfFormId.ExtensionRequest).Count();
            if (countOfExtensionRequest > 0)
            {
                ERPNWorkToDoViewModel model = new ERPNWorkToDoViewModel();
                model = ApplicationService.GetEntities(p => p.ApplicationId == applicationid && p.FormID == Convert.ToInt32(ResearchAwardTypeOfFormId.ExtensionRequest)).FirstOrDefault().ToViewModel();
                if (model.IsTemporary == true)
                {
                    ViewBag.Id = model.ApplicationId;
                    ViewBag.act = "save";
            ...
            {
                int formId = (int)ResearchAwardTypeOfFormId.ExtensionRequest;
                int workflowId = (int)ResearchAwardTypeOfWorkflowId.ExtensionRequest;
        ...
        public ActionResult ExtensionAgree(string id, string nextaction)
        {
            ...
            model = ApplicationService.GetEntities(p => p.ApplicationId == id1 && p.FormID == Convert.ToInt32(ResearchAwardTypeOfFormId.ExtensionRequest)).FirstOrDefault().ToViewModel();
            //继续审批
        ...
        public ActionResult ProcessRecordList(string id)
        {
            ...
            var result = ApplicationService.GetEntities(p => p.StateNow == "正在办理" && p.FormID == (int)ResearchAwardTypeOfFormId.ProcessRecord && p.ApplicationId == applicationid).OrderByDescending(p => p.TimeStr).ToList();
        ...
        {
            var result = ERPRiZhiService.GetEntities(p => p.FKAction == "待办" && p.ModuleName == ModuleNameOfScienceResearch.ResearchAward.ToString()).OrderByDescending(p => p.ID).Take(20);
        ...
        {
            var result = ERPRiZhiService.GetEntities(p => p.FKAction == "已办" && p.ModuleName == ModuleNameOfScienceResearch.ResearchAward.ToString()).OrderByDescending(p => p.ID).Take(20);
            IList<ERPRiZhiViewModel> resultlist = new List<ERPRiZhiViewModel>();
            foreach (var item in result)
            {
                resultlist.Add(item.ToViewModel());
        ...
        {
            var result = ERPRiZhiService.GetEntities(p => p.FKAction == "待办" && p.ModuleName == ModuleNameOfScienceResearch.ResearchAward.ToString()).OrderByDescending(p => p.ID).Take(10);
            IList<ERPRiZhiViewModel> resultlist = new List<ERPRiZhiViewModel>();
        ...
        public ActionResult Top10WorkDoingList()
        {
            ...
            if (User.IsInRole(UserRoles.超级管理员.ToString()))
            {
                result = ApplicationService.GetPageEntities(p => p.StateNow == "正在办理"
                    && p.FormID == (int)ResearchAwardTypeOfFormId.Application
                    && p.ProjectStatus == ResearchAwardApplicationStatus.BigProjectProcessing.ToString(), ApplicationSortField.TimeStr_Desc.ToString(), Constant.PageSize, 1, out totalPage);
            }
            else
            {
                result = ApplicationService.GetPageEntities(p => p.StateNow == "正在办理"
        ...
        {
            var result = ProjectBonusCreditService.GetEntities(p => p.ModuleName == "科教成果奖").OrderByDescending(p => p.Id);
        ...
        public ActionResult SubmitApplication(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            //加载表单内容
            var temerpnformmodel = ERPNFormService.GetEntityById((int)ResearchAwardTypeOfFormId.Application).ToViewModel();
            ...
            string formKeys = "项目类型#奖励级别#申请时间#申请人#科室#项目负责人#项目参与人员#概述";
            string programeType = collection["Drop1364262284"].ToString();
            string prjectScienceResearchType = collection["Drop968600384"].ToString();
            ...
            string formvalues = programeType + "#" + prjectScienceResearchType + "#" + applyDate + "#" + applyMan + "#" + department + "#" + programLeader + "#" + temMenber + "#" + summarize;
            ...
            if (flag == "Save")//保存
            {
                ...
                //申请书状态
                model.ApplicationStatus = ResearchAwardApplicationStatus.AplicationWriting.ToString();
                //整个项目进行的状态
                model.ProjectStatus = ResearchAwardApplicationStatus.AplicationWriting.ToString();
                ...
                MyRiZhi.DoSomething = "保存(科技成果申请书)";
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.FkFormName = ResearchAwardTypeOfFormId.Application.ToString();
                MyRiZhi.FKAction = "已办";
            ...
            else//上报
            {
                ...
                {
                    model.ApplicationStatus = ResearchAwardApplicationStatus.ApplicationApproving.ToString();
                    model.ProjectStatus = ResearchAwardApplicationStatus.ApplicationApproving.ToString();
                    ...
                    //已办
                    MyRiZhi.UserName = User.Identity.Name;
                    MyRiZhi.DoSomething = User.Identity.Name + "上报(科技成果申请书)";
                    MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                    // MyRiZhi.NotificationContent = "添加的" + model.WenHao + "已经提交";
                    MyRiZhi.FkFormName = ResearchAwardTypeOfFormId.Application.ToString();
                    MyRiZhi.FKAction = "已办";
                    MyRiZhi.FKApplicationID = nworktodoid.ToString();
                    MyRiZhi.TimeStr = DateTime.Now; MyRiZhi.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
                    ...
                    MyRiZhi1.DoSomething = "需要审批(" + User.Identity.Name + "上报的科技成果申请书)";
                    ...
                    MyRiZhi1.FkFormName = ResearchAwardTypeOfFormId.Application.ToString();
                    MyRiZhi1.FKAction = "待办";
                    MyRiZhi1.FKApplicationID = nworktodoid.ToString();
                    MyRiZhi1.TimeStr = DateTime.Now; MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
        ...
        public ActionResult ApplicationAgree(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            if (flag == "Approval")
            {
                ...
                if (nextNodeModel.Count == Constant.ZERO_INT)
                {
                    // 说明审批流程已经完成，审批通过
                    model.JieDianName = "结束";
                    model.ApplicationStatus = ResearchAwardApplicationStatus.ApplicationApproved.ToString();//申请书审批完成
                    model.ProjectStatus = ResearchAwardApplicationStatus.ApplicationApproved.ToString();
                ...
                else
                {
                    ...
                    model.ApplicationStatus = ResearchAwardApplicationStatus.ApplicationApproving.ToString();//申请书待审批中 
                    model.ProjectStatus = ResearchAwardApplicationStatus.ApplicationApproving.ToString();
            ...
            else
            {
                ...
                model.ApplicationStatus = ResearchAwardApplicationStatus.ApplicationRejected.ToString();//申请书审批完成
                model.ProjectStatus = ResearchAwardApplicationStatus.ApplicationRejected.ToString();
            ...
            if (nodeSerilsList.Contains(nodeSerils))
            {
                ...
                MyRiZhi.UserName = User.Identity.Name;
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.DoSomething = "已审批" + model.UserName + "上报的科技成果申请书";
                MyRiZhi.FkFormName = ResearchAwardTypeOfFormId.Application.ToString();
                MyRiZhi.FKApplicationID = model.NWorkToDoID.ToString();
                MyRiZhi.TimeStr = DateTime.Now; MyRiZhi.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
                ...
                MyRiZhi1.FKApplicationID = model.NWorkToDoID.ToString();
                MyRiZhi1.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi1.DoSomething = "需要审批(" + model.UserName + "上报的科技成果申请书)";
                MyRiZhi1.FkFormName = ResearchAwardTypeOfFormId.Application.ToString();
                MyRiZhi1.TimeStr = DateTime.Now; MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
            ...
            else if (nodeSerils == "结束")
            {
                ...
                MyRiZhi.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
                ...
                MyRiZhi1.FkFormName = ResearchAwardTypeOfFormId.Establishment.ToString();
                MyRiZhi1.TimeStr = DateTime.Now; MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
            ...
            else
            {
                ...
                MyRiZhi.UserName = User.Identity.Name;
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.DoSomething = "已驳回" + model.UserName + "上报的科技成果申请书";
                MyRiZhi.FkFormName = ResearchAwardTypeOfFormId.Application.ToString();
                MyRiZhi.FKApplicationID = model.NWorkToDoID.ToString();
                MyRiZhi.TimeStr = DateTime.Now; MyRiZhi.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
                ...
                MyRiZhi1.UserName = model.UserName;
                MyRiZhi1.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi1.DoSomething = "科技成果申请书被驳回，需修改数据";
                MyRiZhi1.FkFormName = ResearchAwardTypeOfFormId.Application.ToString();
                MyRiZhi1.FKApplicationID = model.NWorkToDoID.ToString();
                MyRiZhi1.TimeStr = DateTime.Now; MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
        ...
        public ActionResult ApplicationRejected(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            if (flag == "Save")//根据已有ID保存更新
            {
                ...
                //申请书状态
                model.ApplicationStatus = ResearchAwardApplicationStatus.ApplicationRejected.ToString();
                //整个项目进行的状态
                model.ProjectStatus = ResearchAwardApplicationStatus.ApplicationRejected.ToString();
        ...
        public ActionResult ProjectEstablish(string flag, ProjectRecordViewModel projectModel, FormCollection collection)
        {
            ...
            if (flag=="Save")
            {
                ...
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.FkFormName = ResearchAwardTypeOfFormId.Establishment.ToString();
                MyRiZhi.FKAction = "已办";
                ...
                if (projectModel.PojectEstablishID == 0)
                {
                    ...
                    erpnworktodomodel.ApplicationStatus = ResearchAwardApplicationStatus.ProjectEstablishing.ToString();
                    erpnworktodomodel.ProjectStatus = ResearchAwardApplicationStatus.ProjectEstablishing.ToString();
            ...
            else
            {
                ...
                //待办变成已办
                string formname = ResearchAwardTypeOfFormId.Establishment.ToString();
                ...
                MyRiZhi1.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi1.FkFormName = ResearchAwardTypeOfFormId.Contract.ToString();
                MyRiZhi1.FKAction = "待办";
                ...
                if (projectModel.PojectEstablishID == 0)
                {
                    ...
                    erpnworktodomodel.ApplicationStatus = ResearchAwardApplicationStatus.ProjectEstablished.ToString();
                    erpnworktodomodel.ProjectStatus = ResearchAwardApplicationStatus.BigProjectProcessing.ToString();
        ...
        public ActionResult SignContract(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            //加载表单内容
            var temerpnformmodel = ERPNFormService.GetEntityById((int)ResearchAwardTypeOfFormId.Contract).ToViewModel();
            model.FormContent = temerpnformmodel.ContentStr;
            model.FormContent = model.FormContent.Replace(Constant.MacroSectionString, sectionName);
            model.FormContent = model.FormContent.Replace(Constant.MacroUserNameString, User.Identity.Name);
            ...
            if (flag == "Save")//保存
            {
                ...
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.FkFormName = ResearchAwardTypeOfFormId.Contract.ToString();
                MyRiZhi.FKAction = "已办";
                ...
                if (model.NWorkToDoID == 0)
                {
                    ...
                    erpnworktodomodel.ApplicationStatus = ResearchAwardApplicationStatus.ContractSigning.ToString();
                    erpnworktodomodel.ProjectStatus = ResearchAwardApplicationStatus.BigProjectProcessing.ToString();
                    bool isUpdateSuccess = ApplicationService.UpdateApplication(erpnworktodomodel.ToDataTransferObjectModel());
            ...
            else 
            {
                ...
                //待办变成已办
                string formname = ResearchAwardTypeOfFormId.Contract.ToString();
                ...
                if (model.NWorkToDoID == 0)
                {
                    ...
                    erpnworktodomodel.ApplicationStatus = ResearchAwardApplicationStatus.ContractSigned.ToString();
                    erpnworktodomodel.ProjectStatus = ResearchAwardApplicationStatus.BigProjectProcessing.ToString();
                    bool isUpdateSuccess = ApplicationService.UpdateApplication(erpnworktodomodel.ToDataTransferObjectModel());
        ...
        public ActionResult ProcessRecords(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            //加载表单内容
            var temerpnformmodel = ERPNFormService.GetEntityById((int)ResearchAwardTypeOfFormId.ProcessRecord).ToViewModel();
            model.FormContent = temerpnformmodel.ContentStr;
            model.FormContent = model.FormContent.Replace(Constant.MacroSectionString, sectionName);
            model.FormContent = model.FormContent.Replace(Constant.MacroUserNameString, User.Identity.Name);
            ...
            if (flag == "Save")//保存
            {
                ...
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.FkFormName = ResearchAwardTypeOfFormId.ProcessRecord.ToString();
                MyRiZhi.FKAction = "已办";
                MyRiZhi.FKApplicationID = model.ApplicationId.ToString();
                MyRiZhi.TimeStr = DateTime.Now;
                MyRiZhi.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
                ...
                tempmodel.ApplicationStatus = ResearchAwardApplicationStatus.ProjectProcessing.ToString();
                tempmodel.ProjectStatus = ResearchAwardApplicationStatus.BigProjectProcessing.ToString();
            ...
            else 
            {
                ...
                MyRiZhi1.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi1.FkFormName = ResearchAwardTypeOfFormId.ProcessRecord.ToString();
                MyRiZhi1.FKAction = "待办";
                MyRiZhi1.FKApplicationID = model.ApplicationId.ToString();
                MyRiZhi1.TimeStr = DateTime.Now; 
                MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
        ...
        public ActionResult processAgree(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            string formname = ResearchAwardTypeOfFormId.ProcessRecord.ToString();
            var rizhiresult = ERPRiZhiService.GetEntities(p => p.FKApplicationID == model.NWorkToDoID.ToString() && p.FkFormName == formname && p.FKAction == "待办").FirstOrDefault();
            ERPNWorkToDoViewModel erpnworktodoModel = new ERPNWorkToDoViewModel();
            erpnworktodoModel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
            IEnumerable<ERPNWorkFlowNodeViewModel> workFlowNodeModelList = new List<ERPNWorkFlowNodeViewModel>();
        ...
        public ActionResult ReimburseProcess(FundsRecordViewModel model, FormCollection collection)
        {
            ...
            fundModel = FundsThresholdService.GetEntities(p => p.FundsType == type && p.ModuleName == ModuleNameOfScienceResearch.ResearchAward.ToString() && p.ProjectType == projectType).FirstOrDefault().ToViewModel();
            ...
            else
            {
                ...
                if (collection["Save"] != null)//保存
                {
                    ...
                    MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                    MyRiZhi.FkFormName = ResearchAwardTypeOfFormId.FeeReimbursement.ToString();
                    MyRiZhi.FKAction = "已办";
                    MyRiZhi.TimeStr = DateTime.Now;
                    MyRiZhi.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
                    ...
                    if (model.FundsRecordID == 0)
                    {
                        ...
                        model.ModuleName = ApplicationType.ResearchAward.ToString();
                ...
                else
                {
                    ...
                    MyRiZhi1.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                    MyRiZhi1.FkFormName = ResearchAwardTypeOfFormId.FeeReimbursement.ToString();
                    MyRiZhi1.FKAction = "待办";
                    MyRiZhi1.TimeStr = DateTime.Now;
                    MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
        ...
        public ActionResult ReimburseAgree(string flag, FundsRecordViewModel model, FormCollection collection)
        {
            ...
            //经费报销单的日志
            if (model.WorkflowId == Convert.ToInt16(ResearchAwardTypeOfWorkflowId.FeeReimbursement))
            {
                string formname = ResearchAwardTypeOfFormId.FeeReimbursement.ToString();
            ...
            {
                string formname = ResearchAwardTypeOfFormId.TravelReimbursement.ToString();
                var rizhiresult = ERPRiZhiService.GetEntities(p => p.FKApplicationID == model.FundsRecordID.ToString() && p.FkFormName == formname && p.FKAction == "待办").FirstOrDefault();
                ERPNWorkToDoViewModel erpnworktodoModel = new ERPNWorkToDoViewModel();
                erpnworktodoModel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                IEnumerable<ERPNWorkFlowNodeViewModel> workFlowNodeModelList = new List<ERPNWorkFlowNodeViewModel>();
                ...
                if (nodeSerilsList.Contains(nodeSerils))
                {
                    ...
                    MyRiZhi.FkFormName = ResearchAwardTypeOfFormId.TravelReimbursement.ToString();
                    //FundsRecord的ID
                    MyRiZhi.FKApplicationID = model.FundsRecordID.ToString();
                    MyRiZhi.TimeStr = DateTime.Now;
                    MyRiZhi.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
                    ...
                    MyRiZhi1.FkFormName = ResearchAwardTypeOfFormId.TravelReimbursement.ToString();
                    MyRiZhi1.TimeStr = DateTime.Now; MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
        ...
        public ActionResult TravelExpensesRejected(TravelFundsRecordViewModel model, FormCollection collection)
        {
            ...
            model.Quantity = '1';
            model.Unit = "元";
            model.WorkflowId = (int)ResearchAwardTypeOfWorkflowId.TravelReimbursement;
            model.Type = "差旅报销单";
        ...
        public ActionResult Conclusions(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            //加载表单内容
            var temerpnformmodel = ERPNFormService.GetEntityById((int)ResearchAwardTypeOfFormId.Conclusion).ToViewModel();
            model.FormContent = temerpnformmodel.ContentStr;
            model.FormContent = model.FormContent.Replace(Constant.MacroSectionString, sectionName);
            model.FormContent = model.FormContent.Replace(Constant.MacroUserNameString, User.Identity.Name);
            model.FormContent = FormContentReplaceHelper.ReplaceFormContentValue(model.FormContent, collection);
            ...
            if (flag == "Save")
            {
                ...
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.FkFormName = ResearchAwardTypeOfFormId.Conclusion.ToString();
                MyRiZhi.FKAction = "已办";
                MyRiZhi.FKApplicationID = model.ApplicationId.ToString();
                MyRiZhi.TimeStr = DateTime.Now; 
                MyRiZhi.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
                ...
                if (model.NWorkToDoID == 0)
                {
                    ...
                    tempmodel.ApplicationStatus = ResearchAwardApplicationStatus.ConcludeUnSubmit.ToString();
                    tempmodel.ProjectStatus = ResearchAwardApplicationStatus.BigProjectProcessing.ToString();
                    bool isupdate = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
            ...
            else 
            {
                ...
                MyRiZhi1.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi1.FkFormName = ResearchAwardTypeOfFormId.Conclusion.ToString();
                MyRiZhi1.FKAction = "待办";
                MyRiZhi1.FKApplicationID = model.ApplicationId.ToString();
                MyRiZhi1.TimeStr = DateTime.Now; 
                MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
                ...
                if (model.NWorkToDoID == 0)
                {
                    ...
                    ERPNWorkToDoViewModel tempmodel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                    tempmodel.ApplicationStatus = ResearchAwardApplicationStatus.ProjectConcluding.ToString();
                    tempmodel.ProjectStatus = ResearchAwardApplicationStatus.BigProjectProcessing.ToString();
                    bool isupdate = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
        ...
        public ActionResult ConclusionsAgree(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            if (flag == "Approval")
            {
                ...
                if (nextNodeModel.Count == Constant.ZERO_INT)
                {
                    ...
                    ERPNWorkToDoViewModel tempmodel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                    tempmodel.ApplicationStatus = ResearchAwardApplicationStatus.ProjectConcluded.ToString();
                    tempmodel.ProjectStatus = ResearchAwardApplicationStatus.BigProjectProcessing.ToString();
                    bool isupdate = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
            ...
            string formname = ResearchAwardTypeOfFormId.Conclusion.ToString();
            var rizhiresult = ERPRiZhiService.GetEntities(p => p.FKApplicationID == model.ApplicationId.ToString() && p.FkFormName == formname && p.FKAction == "待办").FirstOrDefault();
            ERPNWorkToDoViewModel erpnworktodoModel = new ERPNWorkToDoViewModel();
            erpnworktodoModel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
            IEnumerable<ERPNWorkFlowNodeViewModel> workFlowNodeModelList = new List<ERPNWorkFlowNodeViewModel>();
        ...
        public ActionResult ConclusionsRejected(ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            if (collection["Save"] != null)
            {
                ...
                ERPNWorkToDoViewModel tempmodel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                tempmodel.ApplicationStatus = ResearchAwardApplicationStatus.ConcludeRejected.ToString();
                tempmodel.ProjectStatus = ResearchAwardApplicationStatus.BigProjectProcessing.ToString();
                bool isupdate = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
        ...
        public ActionResult ExtensionRequest(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            //加载表单内容
            var temerpnformmodel = ERPNFormService.GetEntityById((int)ResearchAwardTypeOfFormId.ExtensionRequest).ToViewModel();
            model.FormContent = temerpnformmodel.ContentStr;
            model.FormContent = model.FormContent.Replace(Constant.MacroSectionString, sectionName);
            model.FormContent = model.FormContent.Replace(Constant.MacroUserNameString, User.Identity.Name);
            model.FormContent = FormContentReplaceHelper.ReplaceFormContentValue(model.FormContent, collection);
            ...
            if (flag == "Save")
            {
                ...
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.FkFormName = ResearchAwardTypeOfFormId.ExtensionRequest.ToString();
                MyRiZhi.FKAction = "已办";
                MyRiZhi.FKApplicationID = model.ApplicationId.ToString();
                int returnRiZhiId = ERPRiZhiService.AddERPRiZhi(MyRiZhi.ToDataTransferObjectModel());
                //第一次保存
                if (model.NWorkToDoID == 0)
                {
                    ...
                    tempmodel.IsTemporary = true;
                    tempmodel.IsDeleted = false;
                    tempmodel.ApplicationStatus = ResearchAwardApplicationStatus.ExtensionUnWrite.ToString();
                    tempmodel.ProjectStatus = ResearchAwardApplicationStatus.BigProjectProcessing.ToString();
                    bool isupdate = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
            ...
            else 
            {
                ...
                MyRiZhi1.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi1.FkFormName = ResearchAwardTypeOfFormId.ExtensionRequest.ToString();
                MyRiZhi1.FKAction = "待办";
                MyRiZhi1.FKApplicationID = model.ApplicationId.ToString();
                MyRiZhi1.TimeStr = DateTime.Now; 
                MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.ResearchAward.ToString();
                ...
                if (model.NWorkToDoID == 0)
                {
                    ...
                    ERPNWorkToDoViewModel tempmodel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                    tempmodel.ApplicationStatus = ResearchAwardApplicationStatus.ExtensionRequestApproving.ToString();
                    tempmodel.ProjectStatus = ResearchAwardApplicationStatus.BigProjectProcessing.ToString();
                    bool isupdate1 = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
        ...
        public ActionResult ExtensionAgree(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            if (flag == "Approval")
            {
                ...
                if (nextNodeModel.Count == Constant.ZERO_INT)
                {
                    ...
                    ERPNWorkToDoViewModel tempmodel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                    tempmodel.ApplicationStatus = ResearchAwardApplicationStatus.ExtensionAgreed.ToString();
                    tempmodel.ProjectStatus = ResearchAwardApplicationStatus.BigProjectProcessing.ToString();
                    bool isupdate1 = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
            ...
            string formname = ResearchAwardTypeOfFormId.ExtensionRequest.ToString();
            var rizhiresult = ERPRiZhiService.GetEntities(p => p.FKApplicationID == model.ApplicationId.ToString() && p.FkFormName == formname && p.FKAction == "待办").FirstOrDefault();
            ERPNWorkToDoViewModel erpnworktodoModel = new ERPNWorkToDoViewModel();
            erpnworktodoModel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
            IEnumerable<ERPNWorkFlowNodeViewModel> workFlowNodeModelList = new List<ERPNWorkFlowNodeViewModel>();
        ...
        public ActionResult ApplicationListStatistics(string startTime, string endTime, string projectName, string State, string Freeze, int page, int pageSize)
        {
            ...
            if (hasRolesFlag)
            {
                resultpage = ApplicationService.GetEntities(p => p.FormID == (int)ResearchAwardTypeOfFormId.Application
                    ...
                    && p.ProjectStatus != ResearchAwardApplicationStatus.BigProjectProcessing.ToString());
        ...
        private IEnumerable<ERPNWorkToDoTransferObject> SearchProcessingApplicationList(string projectName, string state, string freeze, DateTime start, DateTime end, int pageSize, int pageIndex, ref int totalPage)
        {
            ...
            if (hasRolesFlag)
            {
                result = ApplicationService.GetPageEntities(p => p.FormID == (int)ResearchAwardTypeOfFormId.Application
                    && p.TimeStr.Value > start
                    && p.TimeStr.Value < end
                    && ((state == Constant.All) ? true : p.ApplicationStatus == state)
                    && (string.IsNullOrEmpty(projectName) ? true : (p.WenHao.Contains(projectName)))
                    && p.StateNow == "正在办理"
                    && ((freeze == Constant.All) ? true : p.IsLocked == (freeze == "冻结"))
                    && p.ProjectStatus != ResearchAwardApplicationStatus.BigProjectProcessing.ToString(), ApplicationSortField.TimeStr_Desc.ToString(), pageSize, pageIndex, out totalPage);
            }
           //普通用户
           else
           {
