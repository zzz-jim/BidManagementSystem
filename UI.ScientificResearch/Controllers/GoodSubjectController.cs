...
namespace UI.ScientificResearch.Controllers
{
    ...
    public class GoodSubjectController : Controller
    {
        ...
        {
            int formId = (int)GoodSubjectTypeOfFormId.Application;
            int workflowId = (int)GoodSubjectTypeOfWorkflowId.Application;
            ...
            else//新建申请书的view
            {
                ...
                // 自动生成 编号
                model.BeiYong1 = CommonHelper.GenerateProjectNumber(ApplicationType.GoodSubject);
        ...
        public ActionResult SignContract(string id, string nextaction)
        {
            ...
            {
                int formId = (int)GoodSubjectTypeOfFormId.Contract;
                int workflowId = (int)GoodSubjectTypeOfWorkflowId.Contract;
                //加载表单内容
                ...
                var result = ApplicationService.GetEntities(p => p.ApplicationId == applicaionId && p.FormID == (int)GoodSubjectTypeOfFormId.Contract).ToList();
        ...
        public ActionResult ProcessRecords(string id, string nextaction)
        {
            ...
            if (workToDoModel.ApplicationId == 0)
            {
                ...
                int formId = (int)GoodSubjectTypeOfFormId.ProcessRecord;
                int workflowId = (int)GoodSubjectTypeOfWorkflowId.ProcessRecord;

                ///数据准备
                ERPNWorkToDoViewModel model = new ERPNWorkToDoViewModel();
                ERPNWorkFlowNodeTransferObject currentNode;
                var appresult = ApplicationService.GetEntities(p => p.ID == applicationid).ToList();
                string projectname = appresult[0].BeiYong1;//课题编号
                string name = appresult[0].WenHao;//项目名称
                var keyvaluearry = appresult[0].FormValues.Split(Constant.SharpChar);
        ...
        public ActionResult ReimburseProcess(string id, string nextaction)
        {
            ...
            if (nextaction == "save")
            {
                ...
                using (var context = new CSPostOAEntities())
                {
                    ViewBag.FundsType = (from h in context.FundsThreshold
                                         where (h.IsDeleted == false && h.ModuleName == ModuleNameOfScienceResearch.GoodSubject.ToString() && h.ProjectType == projectType)
            ...
            else
            {
                ...
                int workflowId = (int)GoodSubjectTypeOfWorkflowId.FeeReimbursement;
                model.CreatedTime = DateTime.Now;
        ...
        public ActionResult TravelExpenses(string id, string nextaction)
        {
            ...
            {
                TravelFundsRecordViewModel model = new TravelFundsRecordViewModel();
                int workflowId = (int)GoodSubjectTypeOfWorkflowId.TravelReimbursement;
        ...
        public ActionResult ConclusionsAgree(string id, string nextaction)
        {
            ...
            var model = ApplicationService.GetEntities(p => p.ApplicationId == id1 && p.FormID == Convert.ToInt32(GoodSubjectTypeOfFormId.Conclusion)).FirstOrDefault().ToViewModel();
            //继续审批
        ...
        public ActionResult Conclusions(string id, string nextaction)
        {
            ...
            //判断是否已经进入到项目延期审批
            int countOfExtensionRequest = ApplicationService.GetEntities(p => p.ID == applicationid && p.ApplicationStatus == GoodSubjectApplicationStatus.ExtensionRequestApproving.ToString()).Count();
            ...
            //判断是否已经进入到项目结题审批中
            var applicaitonodelcount = ApplicationService.GetEntities(p => p.ApplicationId == applicationid && p.FormID == (int)GoodSubjectTypeOfFormId.Conclusion).Count();

            if (applicaitonodelcount > 0)
            {
                ERPNWorkToDoViewModel model = new ERPNWorkToDoViewModel();
                model = ApplicationService.GetEntities(p => p.ApplicationId == applicationid && p.FormID == Convert.ToInt32(GoodSubjectTypeOfFormId.Conclusion)).FirstOrDefault().ToViewModel();
            ...
            if (nextaction == "save")
            {
                ...
                ViewBag.act = "save";
                ERPNWorkToDoViewModel model = new ERPNWorkToDoViewModel();
                model = ApplicationService.GetEntities(p => p.FormID == (int)GoodSubjectTypeOfFormId.Conclusion && p.ApplicationId == applicationid).FirstOrDefault().ToViewModel();
                return View(model);
            ...
            {
                int formId = (int)GoodSubjectTypeOfFormId.Conclusion;
                int workflowId = (int)GoodSubjectTypeOfWorkflowId.Conclusion;
        ...
        public ActionResult ExtensionRequest(string id, string nextaction)
        {
            ...
            int countOfConclusion = ApplicationService.GetEntities(p => p.ApplicationId == applicationid && p.FormID == (int)GoodSubjectTypeOfFormId.Conclusion&&p.IsTemporary!=true).Count();
            ...
            int countOfExtensionRequest = ApplicationService.GetEntities(p => p.ApplicationId == applicationid && p.FormID == (int)GoodSubjectTypeOfFormId.ExtensionRequest).Count();
            if (countOfExtensionRequest > 0)
            {
                ERPNWorkToDoViewModel model = new ERPNWorkToDoViewModel();
                model = ApplicationService.GetEntities(p => p.ApplicationId == applicationid && p.FormID == Convert.ToInt32(GoodSubjectTypeOfFormId.ExtensionRequest)).FirstOrDefault().ToViewModel();
                if (model.IsTemporary == true)
                {
                    ViewBag.Id = model.ApplicationId;
                    ViewBag.act = "save";
            ...
            {
                int formId = (int)GoodSubjectTypeOfFormId.ExtensionRequest;
                int workflowId = (int)GoodSubjectTypeOfWorkflowId.ExtensionRequest;
        ...
        public ActionResult ExtensionAgree(string id, string nextaction)
        {
            ...
            model = ApplicationService.GetEntities(p => p.ApplicationId == id1 && p.FormID == Convert.ToInt32(GoodSubjectTypeOfFormId.ExtensionRequest)).FirstOrDefault().ToViewModel();
            //继续审批
        ...
        public ActionResult ProcessRecordList(string id)
        {
            ...
            var result = ApplicationService.GetEntities(p => p.StateNow == "正在办理" && p.FormID == (int)GoodSubjectTypeOfFormId.ProcessRecord && p.ApplicationId == applicationid).OrderByDescending(p => p.TimeStr).ToList();
        ...
        {
            var result = ERPRiZhiService.GetEntities(p => p.FKAction == "待办" && p.ModuleName == ModuleNameOfScienceResearch.GoodSubject.ToString()).OrderByDescending(p => p.ID).Take(20);
        ...
        {
            var result = ERPRiZhiService.GetEntities(p => p.FKAction == "已办" && p.ModuleName == ModuleNameOfScienceResearch.GoodSubject.ToString()).OrderByDescending(p => p.ID).Take(20);
            IList<ERPRiZhiViewModel> resultlist = new List<ERPRiZhiViewModel>();
            foreach (var item in result)
            {
                resultlist.Add(item.ToViewModel());
        ...
        {
            var result = ERPRiZhiService.GetEntities(p => p.FKAction == "待办" && p.ModuleName == ModuleNameOfScienceResearch.GoodSubject.ToString()).OrderByDescending(p => p.ID).Take(10);
            IList<ERPRiZhiViewModel> resultlist = new List<ERPRiZhiViewModel>();
        ...
        public ActionResult Top10WorkDoingList()
        {
            ...
            if (User.IsInRole(UserRoles.超级管理员.ToString()))
            {
                result = ApplicationService.GetPageEntities(p => p.StateNow == "正在办理"
                    && p.FormID == (int)GoodSubjectTypeOfFormId.Application
                    && p.ProjectStatus == GoodSubjectApplicationStatus.BigProjectProcessing.ToString(), ApplicationSortField.TimeStr_Desc.ToString(), Constant.PageSize, 1, out totalPage);
            }
            else
            {
                result = ApplicationService.GetPageEntities(p => p.StateNow == "正在办理"
        ...
        {
            var result = ProjectBonusCreditService.GetEntities(p => p.ModuleName == "政府采购").OrderByDescending(p => p.Id);
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult SubmitApplication(ERPNWorkToDoViewModel model, FormCollection collection, string flag)
        {
            ///验证
            ...
            //加载表单内容
            var temerpnformmodel = ERPNFormService.GetEntityById((int)GoodSubjectTypeOfFormId.Application).ToViewModel();
            ...
            string formKeys = "项目类型#立项类型#申请时间#申请人#科室#项目负责人#学科带头人#项目参与人员#概述";
            ...
            string academicLeader = collection["Text1885232041"].ToString();
            string temMenber = collection["Text309804476"].ToString();
            string summarize = collection["TextArea683159807"].ToString();
            string formvalues = programeType + "#" + prjectapprovalType + "#" + applyDate + "#" + applyMan + "#" + department + "#" + programLeader + "#" + academicLeader + "#" + temMenber + "#" + summarize;
            ...
            if (flag == "Save")//保存
            {
                ...
                //申请书状态
                model.ApplicationStatus = GoodSubjectApplicationStatus.AplicationWriting.ToString();
                //整个项目进行的状态
                model.ProjectStatus = GoodSubjectApplicationStatus.AplicationWriting.ToString();
                ...
                MyRiZhi.FkFormName = GoodSubjectTypeOfFormId.Application.ToString();
                MyRiZhi.FKAction = "已办";
            ...
            else//上报
            {
                ...
                {
                    model.ApplicationStatus = GoodSubjectApplicationStatus.ApplicationApproving.ToString();
                    model.ProjectStatus = GoodSubjectApplicationStatus.ApplicationApproving.ToString();
                    ...
                    MyRiZhi.FKAction = "已办";
                    MyRiZhi.FKApplicationID = nworktodoid.ToString();
                    MyRiZhi.TimeStr = DateTime.Now; MyRiZhi.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
                    ...
                    MyRiZhi1.FkFormName = GoodSubjectTypeOfFormId.Application.ToString();
                    MyRiZhi1.FKAction = "待办";
                    MyRiZhi1.FKApplicationID = nworktodoid.ToString();
                    MyRiZhi1.TimeStr = DateTime.Now; MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
                ...
                else
                {
                    ...
                    string formname = GoodSubjectTypeOfFormId.Application.ToString();
        ...
        public ActionResult ApplicationAgree(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            if (flag == "Approval")
            {
                ...
                if (nextNodeModel.Count == Constant.ZERO_INT)
                {
                    // 说明审批流程已经完成，审批通过
                    model.JieDianName = "结束";
                    model.ApplicationStatus = GoodSubjectApplicationStatus.ApplicationApproved.ToString();//申请书审批完成
                    model.ProjectStatus = GoodSubjectApplicationStatus.ApplicationApproved.ToString();
                ...
                else
                {
                    ...
                    model.ApplicationStatus = GoodSubjectApplicationStatus.ApplicationApproving.ToString();//申请书待审批中 
                    model.ProjectStatus = GoodSubjectApplicationStatus.ApplicationApproving.ToString();
            ...
            else
            {
                ...
                model.ApplicationStatus = GoodSubjectApplicationStatus.ApplicationRejected.ToString();//申请书审批完成
                model.ProjectStatus = GoodSubjectApplicationStatus.ApplicationRejected.ToString();
            ...
            else if (nodeSerils == "结束")
            {
                ...
                MyRiZhi1.FkFormName = GoodSubjectTypeOfFormId.Establishment.ToString();
                MyRiZhi1.TimeStr = DateTime.Now; MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
            ...
            else
            {
                ...
                MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
        ...
        public ActionResult ApplicationRejected(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            if (flag == "Save")//根据已有ID保存更新
            {
                ...
                //申请书状态
                model.ApplicationStatus = GoodSubjectApplicationStatus.ApplicationRejected.ToString();
                //整个项目进行的状态
                model.ProjectStatus = GoodSubjectApplicationStatus.ApplicationRejected.ToString();
                ...
                MyRiZhi.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
                MyRiZhi.FKApplicationID = model.NWorkToDoID.ToString();
        ...
        public ActionResult ProjectEstablish(string flag, ProjectRecordViewModel projectModel, FormCollection collection)
        {
            ...
            if (flag == "Save")
            {
                ...
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.FkFormName = GoodSubjectTypeOfFormId.Establishment.ToString();
                MyRiZhi.FKAction = "已办";
                ...
                if (projectModel.PojectEstablishID == 0)
                {
                    ...
                    //更新申请书中的GoodSubjectApplicationStatus
                    erpnworktodomodel.ApplicationStatus = GoodSubjectApplicationStatus.ProjectEstablishing.ToString();
                    erpnworktodomodel.ProjectStatus = GoodSubjectApplicationStatus.ProjectEstablishing.ToString();
            ...
            else 
            {
                ...
                //待办变成已办
                string formname = GoodSubjectTypeOfFormId.Establishment.ToString();
                ...
                MyRiZhi1.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi1.FkFormName = GoodSubjectTypeOfFormId.Contract.ToString();
                MyRiZhi1.FKAction = "待办";
                ...
                if (projectModel.PojectEstablishID == 0)
                {
                    ...
                    erpnworktodomodel.ApplicationStatus = GoodSubjectApplicationStatus.ProjectEstablished.ToString();
                    erpnworktodomodel.ProjectStatus = GoodSubjectApplicationStatus.BigProjectProcessing.ToString();
                    //更新申请书中GoodSubjectApplicationStatus的状态为ProjectEstablished
        ...
        public ActionResult SignContract(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            //加载表单内容
            var temerpnformmodel = ERPNFormService.GetEntityById((int)GoodSubjectTypeOfFormId.Contract).ToViewModel();
            model.FormContent = temerpnformmodel.ContentStr;
            model.FormContent = model.FormContent.Replace(Constant.MacroSectionString, sectionName);
            model.FormContent = model.FormContent.Replace(Constant.MacroUserNameString, User.Identity.Name);
            model.FormContent = FormContentReplaceHelper.ReplaceFormContentValue(model.FormContent, collection);
            ...
            if (flag == "Save")//保存
            {
                ...
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.FkFormName = GoodSubjectTypeOfFormId.Contract.ToString();
                MyRiZhi.FKAction = "已办";
                ...
                if (model.NWorkToDoID == 0)
                {
                    ...
                    //更新申请书中的GoodSubjectApplicationStatus为ContractSigning
                    ERPNWorkToDoViewModel erpnworktodomodel = new ERPNWorkToDoViewModel();
                    erpnworktodomodel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                    erpnworktodomodel.ApplicationStatus = GoodSubjectApplicationStatus.ContractSigning.ToString();
                    erpnworktodomodel.ProjectStatus = GoodSubjectApplicationStatus.BigProjectProcessing.ToString();
                    bool isUpdateSuccess = ApplicationService.UpdateApplication(erpnworktodomodel.ToDataTransferObjectModel());
            ...
            else 
            {
                ...
                //待办变成已办
                string formname = GoodSubjectTypeOfFormId.Contract.ToString();
                ...
                if (model.NWorkToDoID == 0)
                {
                    ...
                    //更新申请书中的GoodSubjectApplicationStatus为ProjectProcessing
                    erpnworktodomodel.ApplicationStatus = GoodSubjectApplicationStatus.ContractSigned.ToString();
                    erpnworktodomodel.ProjectStatus = GoodSubjectApplicationStatus.BigProjectProcessing.ToString();
                    bool isUpdateSuccess = ApplicationService.UpdateApplication(erpnworktodomodel.ToDataTransferObjectModel());
        ...
        public ActionResult ProcessRecords(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            //加载表单内容
            var temerpnformmodel = ERPNFormService.GetEntityById((int)GoodSubjectTypeOfFormId.ProcessRecord).ToViewModel();
            model.FormContent = temerpnformmodel.ContentStr;
            model.FormContent = model.FormContent.Replace(Constant.MacroSectionString, sectionName);
            model.FormContent = model.FormContent.Replace(Constant.MacroUserNameString, User.Identity.Name);
            ...
            if (flag == "Save")
            {
                ...
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.FkFormName = GoodSubjectTypeOfFormId.ProcessRecord.ToString();
                MyRiZhi.FKAction = "已办";
                MyRiZhi.FKApplicationID = model.ApplicationId.ToString();
                MyRiZhi.TimeStr = DateTime.Now;
                MyRiZhi.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
                ...
                //更新过程记录相关申请书的GoodSubjectApplicationStatus
                ERPNWorkToDoViewModel tempmodel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                tempmodel.ApplicationStatus = GoodSubjectApplicationStatus.ProjectProcessing.ToString();
                ...
                int isAddsuccess = 1;
                return Json(isAddsuccess, JsonRequestBehavior.AllowGet);
                //return View("ProcessRecordsDetail", model);
            }
            //上报
            else
            {
                ...
                MyRiZhi1.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi1.FkFormName = GoodSubjectTypeOfFormId.ProcessRecord.ToString();
                MyRiZhi1.FKAction = "待办";
        ...
        public ActionResult processAgree(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            string formname = GoodSubjectTypeOfFormId.ProcessRecord.ToString();
            var rizhiresult = ERPRiZhiService.GetEntities(p => p.FKApplicationID == model.NWorkToDoID.ToString() && p.FkFormName == formname && p.FKAction == "待办").FirstOrDefault();
            ERPNWorkToDoViewModel erpnworktodoModel = new ERPNWorkToDoViewModel();
            erpnworktodoModel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
            IEnumerable<ERPNWorkFlowNodeViewModel> workFlowNodeModelList = new List<ERPNWorkFlowNodeViewModel>();
        ...
        public ActionResult ReimburseProcess(FundsRecordViewModel model, FormCollection collection)
        {
            ...
            fundModel = FundsThresholdService.GetEntities(p => p.FundsType == type && p.ModuleName == ModuleNameOfScienceResearch.GoodSubject.ToString() && p.ProjectType == projectType).FirstOrDefault().ToViewModel();
            ...
            else
            {
                ...
                if (collection["Save"] != null)//保存
                {
                    ...
                    MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                    MyRiZhi.FkFormName = GoodSubjectTypeOfFormId.FeeReimbursement.ToString();
                    MyRiZhi.FKAction = "已办";
                    MyRiZhi.TimeStr = DateTime.Now;
                    MyRiZhi.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
                    ...
                    if (model.FundsRecordID == 0)
                    {
                        ...
                        model.ModuleName = ApplicationType.GoodSubject.ToString();
                ...
                else
                {
                    ...
                    MyRiZhi1.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                    MyRiZhi1.FkFormName = GoodSubjectTypeOfFormId.FeeReimbursement.ToString();
                    MyRiZhi1.FKAction = "待办";
                    MyRiZhi1.TimeStr = DateTime.Now;
                    MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
        ...
        public ActionResult ReimburseAgree(string flag, FundsRecordViewModel model, FormCollection collection)
        {
            ...
            //经费报销单的日志
            if (model.WorkflowId == Convert.ToInt16(GoodSubjectTypeOfWorkflowId.FeeReimbursement))
            {
                string formname = GoodSubjectTypeOfFormId.FeeReimbursement.ToString();
            ...
            {
                string formname = GoodSubjectTypeOfFormId.TravelReimbursement.ToString();
                var rizhiresult = ERPRiZhiService.GetEntities(p => p.FKApplicationID == model.FundsRecordID.ToString() && p.FkFormName == formname && p.FKAction == "待办").FirstOrDefault();
                ERPNWorkToDoViewModel erpnworktodoModel = new ERPNWorkToDoViewModel();
                erpnworktodoModel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                IEnumerable<ERPNWorkFlowNodeViewModel> workFlowNodeModelList = new List<ERPNWorkFlowNodeViewModel>();
                ...
                if (nodeSerilsList.Contains(nodeSerils))
                {
                    ...
                    MyRiZhi.FkFormName = GoodSubjectTypeOfFormId.TravelReimbursement.ToString();
                    //FundsRecord的ID
                    MyRiZhi.FKApplicationID = model.FundsRecordID.ToString();
                    MyRiZhi.TimeStr = DateTime.Now;
                    MyRiZhi.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
                    ...
                    MyRiZhi1.FkFormName = GoodSubjectTypeOfFormId.TravelReimbursement.ToString();
                    MyRiZhi1.TimeStr = DateTime.Now; MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
        ...
        public ActionResult TravelExpensesRejected(TravelFundsRecordViewModel model, FormCollection collection)
        {
            ...
            model.Quantity = '1';
            model.Unit = "元";
            model.WorkflowId = (int)GoodSubjectTypeOfWorkflowId.TravelReimbursement;
            model.Type = "差旅报销单";
        ...
        public ActionResult Conclusions(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            //加载表单内容
            var temerpnformmodel = ERPNFormService.GetEntityById((int)GoodSubjectTypeOfFormId.Conclusion).ToViewModel();
            model.FormContent = temerpnformmodel.ContentStr;
            model.FormContent = model.FormContent.Replace(Constant.MacroSectionString, sectionName);
            model.FormContent = model.FormContent.Replace(Constant.MacroUserNameString, User.Identity.Name);
            model.FormContent = FormContentReplaceHelper.ReplaceFormContentValue(model.FormContent, collection);
            ...
            if (flag == "Save")
            {
                ...
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.FkFormName = GoodSubjectTypeOfFormId.Conclusion.ToString();
                MyRiZhi.FKAction = "已办";
                MyRiZhi.FKApplicationID = model.ApplicationId.ToString();
                MyRiZhi.TimeStr = DateTime.Now; MyRiZhi.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
                ...
                if (model.NWorkToDoID == 0)
                {
                    ...
                    tempmodel.ApplicationStatus = GoodSubjectApplicationStatus.ConcludeUnSubmit.ToString();
                    tempmodel.ProjectStatus = GoodSubjectApplicationStatus.BigProjectProcessing.ToString();
                    bool isupdate = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
            ...
            else 
            {
                ...
                MyRiZhi1.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi1.FkFormName = GoodSubjectTypeOfFormId.Conclusion.ToString();
                MyRiZhi1.FKAction = "待办";
                MyRiZhi1.FKApplicationID = model.ApplicationId.ToString();
                MyRiZhi1.TimeStr = DateTime.Now; 
                MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
                ...
                if (model.NWorkToDoID == 0)
                {
                    ...
                    ERPNWorkToDoViewModel tempmodel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                    tempmodel.ApplicationStatus = GoodSubjectApplicationStatus.ProjectConcluding.ToString();
                    tempmodel.ProjectStatus = GoodSubjectApplicationStatus.BigProjectProcessing.ToString();
                    bool isupdate = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
        ...
        public ActionResult ConclusionsAgree(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            if (flag == "Approval")
            {
                ...
                if (nextNodeModel.Count == Constant.ZERO_INT)
                {
                    ...
                    ERPNWorkToDoViewModel tempmodel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                    tempmodel.ApplicationStatus = GoodSubjectApplicationStatus.ProjectConcluded.ToString();
                    tempmodel.ProjectStatus = GoodSubjectApplicationStatus.BigProjectProcessing.ToString();
                    bool isupdate = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
            ...
            string formname = GoodSubjectTypeOfFormId.Conclusion.ToString();
            var rizhiresult = ERPRiZhiService.GetEntities(p => p.FKApplicationID == model.ApplicationId.ToString() && p.FkFormName == formname && p.FKAction == "待办").FirstOrDefault();
            ERPNWorkToDoViewModel erpnworktodoModel = new ERPNWorkToDoViewModel();
            erpnworktodoModel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
            IEnumerable<ERPNWorkFlowNodeViewModel> workFlowNodeModelList = new List<ERPNWorkFlowNodeViewModel>();
        ...
        public ActionResult ExtensionRequest(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            //加载表单内容
            var temerpnformmodel = ERPNFormService.GetEntityById((int)GoodSubjectTypeOfFormId.ExtensionRequest).ToViewModel();
            model.FormContent = temerpnformmodel.ContentStr;
            model.FormContent = model.FormContent.Replace(Constant.MacroSectionString, sectionName);
            model.FormContent = model.FormContent.Replace(Constant.MacroUserNameString, User.Identity.Name);
            model.FormContent = FormContentReplaceHelper.ReplaceFormContentValue(model.FormContent, collection);
            ...
            if (flag == "Save")
            {
                ...
                MyRiZhi.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi.FkFormName = GoodSubjectTypeOfFormId.ExtensionRequest.ToString();
                MyRiZhi.FKAction = "已办";
                MyRiZhi.FKApplicationID = model.ApplicationId.ToString();
                int returnRiZhiId = ERPRiZhiService.AddERPRiZhi(MyRiZhi.ToDataTransferObjectModel());
                //第一次保存
                if (model.NWorkToDoID == 0)
                {
                    ...
                    tempmodel.IsTemporary = true;
                    tempmodel.IsDeleted = false;
                    tempmodel.ApplicationStatus = GoodSubjectApplicationStatus.ExtensionUnWrite.ToString();
                    tempmodel.ProjectStatus = GoodSubjectApplicationStatus.BigProjectProcessing.ToString();
                    bool isupdate = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
            ...
            else 
            {
                ...
                MyRiZhi1.IpStr = System.Web.HttpContext.Current.Request.UserHostAddress.ToString();
                MyRiZhi1.FkFormName = GoodSubjectTypeOfFormId.ExtensionRequest.ToString();
                MyRiZhi1.FKAction = "待办";
                MyRiZhi1.FKApplicationID = model.ApplicationId.ToString();
                MyRiZhi1.TimeStr = DateTime.Now; MyRiZhi1.ModuleName = ModuleNameOfScienceResearch.GoodSubject.ToString();
                ...
                if (model.NWorkToDoID == 0)
                {
                    ...
                    ERPNWorkToDoViewModel tempmodel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                    tempmodel.ApplicationStatus = GoodSubjectApplicationStatus.ExtensionRequestApproving.ToString();
                    tempmodel.ProjectStatus = GoodSubjectApplicationStatus.BigProjectProcessing.ToString();
                    bool isupdate1 = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
        ...
        public ActionResult ExtensionAgree(string flag, ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            if (flag == "Approval")
            {
                ...
                if (nextNodeModel.Count == Constant.ZERO_INT)
                {
                    ...
                    ERPNWorkToDoViewModel tempmodel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
                    tempmodel.ApplicationStatus = GoodSubjectApplicationStatus.ExtensionAgreed.ToString();
                    tempmodel.ProjectStatus = GoodSubjectApplicationStatus.BigProjectProcessing.ToString();
                    bool isupdate1 = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
            ...
            string formname = GoodSubjectTypeOfFormId.ExtensionRequest.ToString();
            var rizhiresult = ERPRiZhiService.GetEntities(p => p.FKApplicationID == model.ApplicationId.ToString() && p.FkFormName == formname && p.FKAction == "待办").FirstOrDefault();
            ERPNWorkToDoViewModel erpnworktodoModel = new ERPNWorkToDoViewModel();
            erpnworktodoModel = ApplicationService.GetEntityById(model.ApplicationId).ToViewModel();
            IEnumerable<ERPNWorkFlowNodeViewModel> workFlowNodeModelList = new List<ERPNWorkFlowNodeViewModel>();
        ...
        public ActionResult ExtensionRejected(ERPNWorkToDoViewModel model, FormCollection collection)
        {
            ...
            if (collection["Save"] != null)
            {
                ...
                tempmodel.IsTemporary = true;
                tempmodel.IsDeleted = false;
                tempmodel.ApplicationStatus = GoodSubjectApplicationStatus.ExtensionRejected.ToString();
                tempmodel.ProjectStatus = GoodSubjectApplicationStatus.BigProjectProcessing.ToString();
                bool isupdate = ApplicationService.UpdateApplication(tempmodel.ToDataTransferObjectModel());
        ...
        public ActionResult ApplicationListStatistics(string startTime, string endTime, string projectName, string State, string Freeze, int page, int pageSize)
        {
            ...
            if (hasRolesFlag)
            {
                resultpage = ApplicationService.GetEntities(p => p.FormID == (int)GoodSubjectTypeOfFormId.Application
                    ...
                    && p.ProjectStatus != GoodSubjectApplicationStatus.BigProjectProcessing.ToString());
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult ProcessingProjectList(string startTime, string endTime, string projectName, string State, string freeze)
        {
            IEnumerable<ERPNWorkToDoTransferObject> data = new List<ERPNWorkToDoTransferObject>();
            DateTime start, end;
            int totalPage = 1;
            ...
            IEnumerable<ERPNWorkToDoTransferObject> result = SearchProcessingProjectList(projectName, State, freeze, start, end, Constant.PageSize, 1, ref totalPage);
        ...
        public ActionResult ProcessingListStatistics(string startTime, string endTime, string projectName, string State, string Freeze, int page, int pageSize)
        {
            ...
            IEnumerable<ERPNWorkToDoTransferObject> result = SearchProcessingProjectList(projectName, State, Freeze, SearchCriteriaStartTime,
                end, pageSize, page, ref totalPage);
        ...
        private IEnumerable<ERPNWorkToDoTransferObject> SearchProcessingApplicationList(string projectName, string state, string freeze, DateTime start, DateTime end, int pageSize, int pageIndex, ref int totalPage)
        {
            ...
            if (hasRolesFlag)
            {
                result = ApplicationService.GetPageEntities(p => p.FormID == (int)GoodSubjectTypeOfFormId.Application
                    && p.TimeStr.Value > start
                    && p.TimeStr.Value < end
                    && ((state == Constant.All) ? true : p.ApplicationStatus == state)
                    && (string.IsNullOrEmpty(projectName) ? true : (p.WenHao.Contains(projectName)))
                    && p.StateNow == "正在办理"
                    && ((freeze == Constant.All) ? true : p.IsLocked == (freeze == "冻结"))
                    && p.ProjectStatus != GoodSubjectApplicationStatus.BigProjectProcessing.ToString(), ApplicationSortField.TimeStr_Desc.ToString(), pageSize, pageIndex, out totalPage);
            }
            else
            {
               result = ApplicationService.GetPageEntities(p => p.FormID == (int)GoodSubjectTypeOfFormId.Application
